...
class HowItFeels extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    var button =
      Object.keys(this.state.feels).length != 0 ? (
        <TouchableOpacity
          style={styles.button}
          onPress={this._bodyParts.bind(this)}>
          <Text style={styles.buttonText}>Where</Text>
        </TouchableOpacity>
      ) : (
        <View />
      );
    ...
    var arrFeels = Object.getOwnPropertyNames(this.state.feelingData).map(
      key => feelingData[key],
    );
    console.log(arrFeels);
    var feels = this.state.feels;
    var counterText = '';
    if (this.state.feelingCounter !== 0) {
      counterText =
        this.state.feelingCounter == 1
          ? this.state.feelingCounter + ' item'
          : this.state.feelingCounter + ' items';
    }
          ...
          <View style={styles.textContainer}>
            <View style={styles.navRow}>
              <TouchableOpacity
                ...
                style={styles.left}>
                <Text style={styles.title}>{words.howYouFeel.english}</Text>
          ...
          <View style={{flex: 1, alignSelf: 'stretch', alignItems: 'stretch'}}>
            <FlatList
              ...
              renderSeparator={(sectionID, rowID) => (
                <View key={`${sectionID}-${rowID}`} style={styles.lineBreak} />
              )}
            />
          </View>
  ...
  _renderRow({item}) {
    ...
    var defaultStyle =
      language.id == 'default' ? styles.feelingEnglishOnly : styles.feeling;
    ...
    return (
      <View style={styles.row}>
        <TouchableOpacity
          ...
          <Text style={styles.feelingTranslation}>{item.translation}</Text>
        </TouchableOpacity>
        {checkbox}
      </View>
    );
...
//         <Image source={backArrow} resizeMode={'contain'} style={styles.navButton} />
//       </TouchableOpacity>
//     );
//   },
